...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AliSayyah/x-ui-english/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    if [[ $yn =~ "Y"|"y" ]]; then
        ...
        last_version=$(curl -Ls "https://api.github.com/repos/AliSayyah/x-ui-english/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://raw.githubusercontent.com/AliSayyah/x-ui-english/main/config/version)
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/AliSayyah/x-ui-english/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
---------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------   "
    ...
    case "${num}" in
        ...
        14) update_geo ;;
        15) install_bbr ;;
        16) wget -N --no-check-certificate https://raw.githubusercontent.com/AliSayyah/x-ui-english/main/acme.sh && bash acme.sh && before_show_menu ;;
        17) open_ports ;;
